...
interface PropriedadesUploadImagemRender {
  aoSelecionarArquivo: (arquivo: File) => void;
  erro: string | null;
}

export default function UploadImagemRender({ aoSelecionarArquivo, erro }: PropriedadesUploadImagemRender) {
      ...
      >
        ...
        <div>
          <p className="font-titulo text-lg font-semibold text-texto-primario">
            {t("conta.renderizar.upload.tituloArraste")}
          </p>
          <p className="mt-1 text-xs uppercase tracking-wide text-texto-suave">
            {t("conta.renderizar.upload.ou")}
          </p>
        </div>
        ...
        >
          {t("conta.renderizar.upload.botaoSelecionar")}
        </Botao>

        <p className="max-w-sm text-xs leading-relaxed text-texto-suave">
          {t("conta.renderizar.upload.formatosAceitos")}
        </p>
        ...
        <input
          ...
          className="sr-only"
          aria-label={t("conta.renderizar.upload.botaoSelecionar")}
        />
      </motion.div>
