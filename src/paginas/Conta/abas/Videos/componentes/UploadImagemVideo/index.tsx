...
interface PropriedadesUploadImagemVideo {
  aoSelecionarArquivo: (arquivo: File) => void;
  erro: string | null;
}

export default function UploadImagemVideo({ aoSelecionarArquivo, erro }: PropriedadesUploadImagemVideo) {
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        >
          {t("conta.videos.upload.botaoSelecionar")}
        </Botao>

        <p className="max-w-sm text-xs leading-relaxed text-texto-suave">
          {t("conta.videos.upload.formatosAceitos")}
        </p>
        ...
        <input
          ...
          className="sr-only"
          aria-label={t("conta.videos.upload.botaoSelecionar")}
        />
      </motion.div>
